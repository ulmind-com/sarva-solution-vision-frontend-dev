...
import { getSsvplSuperBonusStatus, getSsvplSuperBonusHistory, getSsvplSuperBonusLiveEstimate } from "@/services/userService";
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-yellow-950 dark:text-yellow-50">SSVPL Super Bonus</h1>
                    <p className="text-muted-foreground mt-1">
                        Track your 2% BV Yearly Pool earnings — <strong>25,00,000 BV = 1 Unit</strong>, <strong>No Capping</strong>
                    </p>
                </div>
                <Badge variant="outline" className="text-sm border-yellow-500/30 text-yellow-600 bg-yellow-500/10 px-3 py-1.5 self-start shadow-sm mix-blend-multiply dark:mix-blend-normal">
                    <Star className="h-3.5 w-3.5 mr-1" /> 2% Annual Pool
                </Badge>
            </div>
            ...
            {liveEstimate && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <Card className="glass premium-shadow border-yellow-500/30 bg-gradient-to-br from-yellow-500/5 to-yellow-500/10">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-sm font-medium text-yellow-700 dark:text-yellow-500">2% Pool Value</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-yellow-700 dark:text-yellow-500">₹{(liveEstimate.pool?.poolAmount || liveEstimate.companyBV?.poolAmount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</div>
                            </CardContent>
                        </Card>
                        <Card className="glass premium-shadow border-yellow-500/10">
                            <CardHeader className="pb-2">
                        ...
                        <Card className="glass premium-shadow border-green-500/30 bg-green-500/10 hover:border-green-500/50 transition-colors">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-green-700/80 mt-1">Based on {liveEstimate.projectedUnits || liveEstimate.myEstimate?.myUnits || 0} units (25,00,000 BV each)</p>
                            </CardContent>
                        </Card>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div>
                        <h2 className="text-lg font-semibold mb-3 flex items-center justify-between gap-2">
                            ...
                            <Badge variant="outline" className="text-sm border-yellow-500/20 bg-yellow-500/5 text-yellow-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <Card className="glass premium-shadow border-yellow-500/10">
                                <CardHeader className="pb-2">
                                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-1">
                                        <ArrowLeft className="h-3.5 w-3.5 text-yellow-500" /> Left Branch BV
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                            ...
                            <Card className="glass premium-shadow border-yellow-500/10">
                                <CardHeader className="pb-2">
                                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center justify-end gap-1">
                                        Right Branch BV <ArrowRight className="h-3.5 w-3.5 text-yellow-500" />
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                            ...
                            {/* Personal BV Add-on */}
                            <Card className="glass premium-shadow border-yellow-500/20 bg-yellow-500/5 hover:bg-yellow-500/10 transition-colors">
                                <CardHeader className="pb-2">
                                    <CardTitle className="text-sm font-medium text-yellow-700">Personal Add-on</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="text-2xl font-bold text-yellow-700">
                                        +{(bvBreakdown?.personalBV || 0).toLocaleString()} <span className="text-sm font-normal text-yellow-700/70">BV</span>
                                    </div>
                            ...
                            <Card className="glass premium-shadow border-yellow-500/30 bg-gradient-to-b from-yellow-500/10 to-yellow-500/5">
                                <CardContent className="pt-6">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="text-sm font-medium text-yellow-700 mb-1">Projected Units</p>
                                            <div className="text-4xl font-black text-yellow-600 tracking-tight">
                                                {units?.estimated || units?.projected || 0}
                                            </div>
                                        </div>
                                        <Badge variant="default" className="bg-yellow-500 hover:bg-yellow-600 shadow-md shadow-yellow-500/20">
                                            No Cap
                                        </Badge>
                                    </div>
                                    <p className="text-xs text-yellow-700/80 mt-3 font-medium">
                                        25,00,000 BV = 1 Unit
                                    </p>
                                </CardContent>
                            </Card>
            ...
            <Card className="glass premium-shadow overflow-hidden border-yellow-500/10 mt-8">
                ...
                <CardContent className="p-0">
                            ...
                            <TableBody>
                                {historyData.length > 0 ? (
                                    historyData.map((record, index) => (
                                        <TableRow key={index} className="hover:bg-yellow-50/50 dark:hover:bg-yellow-900/20 transition-colors">
                                            <TableCell className="font-medium whitespace-nowrap">
                                                {formatCycle({ cycleYear: record.cycleYear || record.year })}
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant="outline" className="border-yellow-500/30 bg-yellow-500/10 text-yellow-700 font-bold">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-32 text-center text-muted-foreground">
                                            No annual SSVPL super bonus payout history found.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                    ...
                    {pagination.totalPages > 1 && (
                        <div className="flex items-center justify-between px-4 py-3 border-t bg-yellow-50/30 dark:bg-yellow-950/10">
                            <div className="text-sm text-muted-foreground">
                                Page {pagination.page} of {pagination.totalPages}
                            </div>
                            <div className="flex gap-2">
                                <Button variant="outline" size="sm" className="border-yellow-200 hover:bg-yellow-50" disabled={pagination.page <= 1} onClick={() => fetchData(pagination.page - 1)}>Previous</Button>
                                <Button variant="outline" size="sm" className="border-yellow-200 hover:bg-yellow-50" disabled={pagination.page >= pagination.totalPages} onClick={() => fetchData(pagination.page + 1)}>Next</Button>
                            </div>
                        </div>
                    )}
                </CardContent>
